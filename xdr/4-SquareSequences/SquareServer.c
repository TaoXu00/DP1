...
/* FUNCTION PROTOTYPES */
SOCKET udp_server_init(uint16_t port);
...
main(int argc, char *argv[])
{
	...
	SOCKET			s;			/* socket where server listens */
...
int service (char *buf, int n, struct sockaddr_in *from, int s)
{
	...
	xdrhyper	inseq[MAXSEQLEN];	/* input sequence */
	xdrhyper	outseq[MAXSEQLEN];	/* output sequence */

	/* initialize XDR input stream to point to buf */
	xdrmem_create(&xdrs_in, buf, n, XDR_DECODE);

	/* decode request */
	req.xdrhypersequence_len = MAXSEQLEN;
	req.xdrhypersequence_val = inseq;

/* Alternative solution
	req.xdrhypersequence_len = 0;
	req.xdrhypersequence_val = NULL;
*/
