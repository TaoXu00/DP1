...
main(int argc, char *argv[])
{
	...
	for (;;)
	{
	    ...
	    sscanf(buf, "%u", &input);
	    req.xdrhypersequence_len = 1;
	    req.xdrhypersequence_val = (xdrhyper *)malloc(1*sizeof(xdrhyper));
	    req.xdrhypersequence_val[0] = input;
		...
		/* receive datagram */
			...
			res.request.xdrhypersequence_val=(xdrhyper *)malloc(BUFLEN*sizeof(xdrhyper));
			res.response.xdrhypersequence_len=BUFLEN;
			res.response.xdrhypersequence_val=(xdrhyper *)malloc(BUFLEN*sizeof(xdrhyper));

			if (!xdr_Response(&xdrs_in, &res)) {
				printf("Error in decoding response\n");
			} else {
				input = res.request.xdrhypersequence_len;
				output = res.response.xdrhypersequence_len;
				printf("Request size: %u\n", input);
				printf("Result size: %u\n", output);
				input = res.request.xdrhypersequence_val[0];
				output = res.response.xdrhypersequence_val[0];
				printf("Request: %u\n", input);
				printf("Result: %u\n", output);
			}
			xdr_destroy(&xdrs_in);			
