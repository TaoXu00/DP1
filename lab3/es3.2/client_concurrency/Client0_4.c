...
/*---------socket s read is ready --------------------*/
      ...
      err_msg("(%s)-timeout waitting for server",prog_name);
      break;
     }
  }
 }
...
{         
        ...
        /*   int f=0;
           ...
           n= Readline_unbuffered (s, rbuf,10);
           if(n==0)
            {
               printf("server closed.\n");
               close(s);
